...
- **Throttled updates with dedup**: Rate-limited per `update_interval` with content dict equality check. `flush_unsub` timer fires after cooldown. At most one `_send_update` task runs per entity (`pending_task`); changes landing mid-send set `update_pending` and the newest state is re-sent through the throttle when the in-flight task finishes.
...
- **Burst coalescing**: `_schedule_update` runs at most one in-flight update task per widget; a state change landing mid-send sets `update_pending` so the newest state is re-sent once the in-flight task finishes (same dirty-flag pattern as `ActivityManager._schedule_throttled_update`).
